...
export default function HTMLRenderPage() {
  ...
  const handleRenderURL = async () => {
    ...
    try {
      const result = await HTMLRender.renderURL(url, {
        width: 1080,
        height: 1920,
      ...
      Alert.alert('Success', `Render generated: ${result.width}x${result.height}`);
    } catch (error) {
      Alert.alert('Error', 'Failed to render HTML');
      console.error('HTMLRender Error:', error);
    } finally {
  ...
  const handleGetUserAgent = async () => {
    try {
      const userAgent = await HTMLRender.getUserAgent();
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top, paddingBottom: insets.bottom }]}>
      ...
      <View style={styles.header}>
        <Text style={styles.title}>HTML Render</Text>
        <Text style={styles.subtitle}>HTML rendering with rn-html-render</Text>
      </View>
      ...
      >
        <View style={styles.card}>
          <Text style={styles.heading}>Render Mode</Text>
        ...
        {mode === 'url' && (
          <View style={styles.card}>
            <Text style={styles.heading}>URL Render</Text>
            <Text style={styles.helper}>Render HTML from a URL</Text>
            ...
            <Button
              title={isProcessing ? 'Processing...' : 'Render HTML'}
              onPress={handleRenderURL}
            ...
            {isProcessing && (
              <View style={styles.processingBox}>
                ...
                <Text style={styles.processingText}>Rendering HTML...</Text>
              </View>
            )}

            {renderResult && (
              <View style={styles.resultBox}>
                <Text style={styles.resultHeading}>Render Result:</Text>
                <Text style={styles.resultText}>Width: {renderResult.width}</Text>
                <Text style={styles.resultText}>Height: {renderResult.height}</Text>
                <Text style={styles.resultText}>Format: {renderResult.format}</Text>
                <Text style={styles.resultText}>Size: {renderResult.size} bytes</Text>
              </View>
            )}
          </View>
        )}
        ...
        {mode === 'html' && (
          <View style={styles.card}>
            <Text style={styles.heading}>HTML Render</Text>
            <Text style={styles.helper}>Render HTML content to image</Text>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.featureList}>
            <Text style={styles.featureItem}>🌐 URL to HTML Render</Text>
            <Text style={styles.featureItem}>📄 HTML Content Render</Text>
